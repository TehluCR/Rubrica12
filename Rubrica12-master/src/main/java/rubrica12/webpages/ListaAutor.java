...
public class ListaAutor extends WebPage {
	private static final long serialVersionUID = -1935854748907274886L;
	
	@SpringBean
	AuthorService authorService;
	
	private static final Logger logger = LogManager.getLogger(ListaAutor.class.getName());
	
	private String currentNameSearch = null;
	
	private List listAuthor = Collections.emptyList();
	
	public ListaAutor(PageParameters parameters) {
	...
	private void addForm() {
		Form form = new Form("formListAuthor", new CompoundPropertyModel(new Author())) {
			...
			protected void onSubmit() {
				...
				setResponsePage(ListaAutor.class, pageParameters);
			}
		};
